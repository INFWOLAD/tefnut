...
export function DatePicker(props: DatePickerProps) {
			...
			{mode === 'range' && (
				...
				>
					<Text variant="subtitle" style={{ flex: 1 }}>
						{tempRange.startDate ? `${tempRange.startDate.toLocaleDateString()}` : '起始日期'}
					</Text>
					...
					<Text variant="subtitle" style={{ flex: 1, textAlign: 'right' }}>
						{tempRange.endDate ? `${tempRange.endDate.toLocaleDateString()}` : '结束日期'}
					</Text>
				</View>
			)}
		</View>
	...
	const getBottomSheetTitle = () => {
		if (showMonthPicker) return '选择月份';
		if (showYearPicker) return '选择年份';

		if (mode === 'datetime') {
			return viewMode === 'date' ? '选择日期' : '选择时间';
		}

		if (mode === 'time') return '选择时间';

		if (mode === 'range') return '选择范围';
					...
					>
						...
						{mode === 'datetime' && viewMode === 'date' ? (
							...
							<Button
								...
								style={{ flex: 1 }}
								disabled={mode === 'range' && (!tempRange.startDate || !tempRange.endDate)}
							>
								完成
							</Button>
						)}
